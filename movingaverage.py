...
if submitted:
    # Tampilkan spinner loading saat data diproses
    with st.spinner("Crunching numbers and fetching data... Please wait."):
        ...
        fig.add_trace(go.Scatter(
            x=df['ds'], y=df['y'], mode='lines', name='Actual Price',
            line=dict(color='#dddddd', width=1)))
        fig.add_trace(go.Scatter(
            x=df['ds'], y=df['MA_Short'], mode='lines', name=f'MA{short_window}',
            line=dict(color='#7ed6df')))
        fig.add_trace(go.Scatter(
            x=df['ds'], y=df['MA_Long'], mode='lines', name=f'MA{long_window}',
