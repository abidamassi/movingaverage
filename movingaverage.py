...
st.markdown("""
    ...
    /* Styling tombol submit di sidebar (mengadopsi style dari project portfolio) */
...
if submitted:
    ...
    # --- Calculate Moving Averages & Generate Signals ---
    ...
    fig.add_trace(go.Scatter(
        x=df['ds'], y=df['y'], mode='lines', name='Actual Price', line=dict(color='#dddddd', width=1)))
    fig.add_trace(go.Scatter(
        x=df['ds'], y=df['MA_Short'], mode='lines', name=f'MA{short_window}', line=dict(color='#7ed6df')))
    fig.add_trace(go.Scatter(
        x=df['ds'], y=df['MA_Long'], mode='lines', name=f'MA{long_window}', line=dict(color='#ff7675')))
    fig.add_trace(go.Scatter(
        x=[latest['ds']], y=[latest['y']], mode='markers+text', name='Latest Price',
        marker=dict(size=10, color='#f9ca24', symbol='circle', line=dict(width=2, color='white')),
        text=["Latest"], textposition="top right"))
    fig.add_trace(go.Scatter(
        x=buy_signals['ds'], y=buy_signals['y'], mode='markers', name='Buy Signal',
        marker=dict(symbol='triangle-up', size=12, color='lime', line=dict(color='white', width=1.5))))
    fig.add_trace(go.Scatter(
        x=sell_signals['ds'], y=sell_signals['y'], mode='markers', name='Sell Signal',
